...
#define SBP_PERIODIC_PORT_EVT            0x100
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  HalSensorReadReg(0x12, &tmp_set, 1);
  SerialPrintValue("tmp_set = ", tmp_set, 10);
  HalSensorReadReg(0x13, &control_status, 1);
  SerialPrintValue("control_status = ", control_status, 10);
  //HalSensorWriteReg(0x13, buf, 1);
  //HalSensorReadReg(0x13, &control_status, 1);
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SBP_PERIODIC_IIC_WRITE_PERMIT_EVT )
  {
      ...
      if(P0_6 == 0)
      {
          ...
          //P0_6 = 1;
          HalSensorInit(SLAVE_DEV_ADDR);
          if(g_set_write_flag == 0){
            HalSensorWriteReg(0x10, g_rsv_buff, 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 1){
            HalSensorWriteReg(0x30, &g_rsv_buff[6], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 2){
            HalSensorWriteReg(0x36, &g_rsv_buff[12], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 3){
            HalSensorWriteReg(0x3C, &g_rsv_buff[18], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 4){
            HalSensorWriteReg(0x42, &g_rsv_buff[24], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 5){
            HalSensorWriteReg(0x48, &g_rsv_buff[30], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 6){
            HalSensorWriteReg(0x4E, &g_rsv_buff[36], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 7){
            HalSensorWriteReg(0x54, &g_rsv_buff[42], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 8){
            HalSensorWriteReg(0x5A, &g_rsv_buff[48], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 9){
            HalSensorWriteReg(0x60, &g_rsv_buff[54], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 10){
            HalSensorWriteReg(0x66, &g_rsv_buff[60], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 11){
            HalSensorWriteReg(0x6C, &g_rsv_buff[66], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 12){
            HalSensorWriteReg(0x72, &g_rsv_buff[72], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 13){
            HalSensorWriteReg(0x78, &g_rsv_buff[78], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 14){
            HalSensorWriteReg(0x7E, &g_rsv_buff[84], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 15){
            HalSensorWriteReg(0x84, &g_rsv_buff[90], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 16){
            HalSensorWriteReg(0x8A, &g_rsv_buff[96], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 17){
            HalSensorWriteReg(0x90, &g_rsv_buff[102], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 18){
            HalSensorWriteReg(0x96, &g_rsv_buff[108], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 19){
            HalSensorWriteReg(0x9C, &g_rsv_buff[114], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 20){
            HalSensorWriteReg(0xA2, &g_rsv_buff[120], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 21){
            HalSensorWriteReg(0xA8, &g_rsv_buff[126], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 22){
            HalSensorWriteReg(0xAE, &g_rsv_buff[132], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 23){
            HalSensorWriteReg(0xB4, &g_rsv_buff[138], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 24){
            HalSensorWriteReg(0xBA, &g_rsv_buff[144], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 25){
            HalSensorWriteReg(0xC0, &g_rsv_buff[150], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 26){
            HalSensorWriteReg(0xC6, &g_rsv_buff[156], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 27){
            HalSensorWriteReg(0xCC, &g_rsv_buff[162], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 28){
            HalSensorWriteReg(0xD2, &g_rsv_buff[168], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 29){
            HalSensorWriteReg(0xE0, &g_rsv_buff[174], 6);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 30){
            HalSensorWriteReg(0xF0, &g_rsv_buff[180], 4);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 50 );
          }else if(g_set_write_flag == 31){
            HalSensorWriteReg(0x18, &afterwrite, 1);
            g_set_write_flag++;
            osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT, 10 );
          }else{
            osal_stop_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_IIC_WRITE_PERMIT_EVT);
          }
          P0_7 = 1;
          //P0_6 = 0;
          osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_PORT_EVT, 5 );
      }
      else if(P0_6 == 1){
  ...
  if ( events & SBP_PERIODIC_TIME_DATA_STORE_EVT )
  {
      ...
      HalSensorInit(SLAVE_DEV_ADDR);
      //P0_7: 当app接上，固件读取数据传给app，app处理后把数据给蓝牙，蓝牙在写入iic器件，写完后通过P0_7给MCU一个中断，告知MCU已经写入完成。
      //if(P0_6 == 0)才能对器件进行读写，读写时将P0_6置高，直到读写结束

       /* HalSensorReadReg(0x20, B1_Data, 1);     //B1
        HalSensorReadReg(0x12, B2_Data, 1);     //B2
        HalSensorReadReg(0x21, B3_Data, 1);     //B3
        
        HalSensorWriteReg(0xFE, B1_Data, 1);     //B1
        HalSensorWriteReg(0xFF, B2_Data, 1);     //B2
        HalSensorWriteReg(0x21, B3_Data, 1);     //B3
*/
  ...
  if ( events & SBP_PERIODIC_PORT_EVT )
  {
      P0_7 = 0;
      
      osal_stop_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_PORT_EVT);
      //osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_TIME_DATA_STORE_EVT, 5*60*1000 );
      return ( events ^ SBP_PERIODIC_PORT_EVT );
  }
  ...
  if ( events & SBP_PERIODIC_CONN_READ_EVT )
  {
      ...
      //P0_7: 当app接上，固件读取数据传给app，app处理后把数据给蓝牙，蓝牙在写入iic器件，写完后通过P0_7给MCU一个中断，告知MCU已经写入完成。
      //if(P0_6 == 0)才能对器件进行读写，读写时将P0_6置高，直到读写结束
      if(g_con_read_flag == 0)
      {
        HalSensorReadReg(0x10, g_send_buff, 6);
        g_con_read_flag++;
        osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_CONN_READ_EVT, 50 );
      }else if(g_con_read_flag == 1){
        HalSensorReadReg(0x30, &g_send_buff[6], 168);
        g_con_read_flag++;
        osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_CONN_READ_EVT, 500 );
      }else if(g_con_read_flag == 2){
        HalSensorReadReg(0xE0, &g_send_buff[174], 6);
        g_con_read_flag++;
        osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_CONN_READ_EVT, 50 );
      }else if(g_con_read_flag == 3){
        HalSensorReadReg(0xF0, &g_send_buff[180], 4);
        g_con_read_flag++;
        osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_CONN_READ_EVT, 50 );
      }else{
        g_con_read_flag = 0;
